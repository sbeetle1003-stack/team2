...
def generate_launch_description():
    ...
    # Gazebo Sim does not search ROS 2's lib directory for system plugins by
    # default.  Without this path, the robot spawns visually but
    # gz_ros2_control never creates /controller_manager, so MoveIt can plan
    # PRE_GRASP but has nowhere to send the trajectory.
    gz_control_lib = str(
        Path(get_package_share_directory('gz_ros2_control')).parent.parent
        / 'lib'
    )
    # ros_gz_sim's nested launch file reads os.environ while it builds the
    # Gazebo process environment, so make the path visible there as well as
    # to normal child processes.
    existing_plugin_path = os.environ.get('GZ_SIM_SYSTEM_PLUGIN_PATH', '')
    plugin_path = os.pathsep.join(
        path for path in (gz_control_lib, existing_plugin_path) if path
    )
