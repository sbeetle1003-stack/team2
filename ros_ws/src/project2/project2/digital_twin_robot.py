...
class DigitalTwinRobotSync(Node):
    ...
    def joint_state_callback(self, msg):
        ...
        # 수신한 관절 상태 데이터를 가상 로봇 제어 토픽으로 그대로 퍼블리시
        sync_msg = JointState()
        sync_msg.header = msg.header
        sync_msg.name = msg.name
        sync_msg.position = msg.position
        sync_msg.velocity = msg.velocity
        sync_msg.effort = msg.effort
        
        self.publisher.publish(sync_msg)
